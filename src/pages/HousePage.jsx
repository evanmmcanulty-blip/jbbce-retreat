...
function Payments({ isAccountant, profile }) {
  ...
  async function logPayment(uid) {
    const amt = parseFloat(window.prompt("Total you've sent to Chris so far ($):", ''));
    if (isNaN(amt)) return;
    await setDoc(doc(db,'payments',uid), { uid, logged: amt }, { merge: true });
  }
  async function confirmPayment(uid) {
    const cur = payDocs.find(p=>p.uid===uid);
    const amt = parseFloat(window.prompt('Confirm total received ($):', cur?.logged ?? ''));
    if (isNaN(amt)) return;
    await setDoc(doc(db,'payments',uid), { uid, confirmed: amt }, { merge: true });
  }
  ...
  return (
    <div>
      ...
      {users.map(u => {
        ...
        return (
          <div key={u.uid} className="pay-row">
            ...
            <div style={{textAlign:'right'}}>
              ...
              <div className="btn-row" style={{justifyContent:'flex-end',marginTop:4}}>
                {mine && hasRoom && <button className="btn-mini" onClick={()=>logPayment(u.uid)}>Log payment</button>}
                {isAccountant && <button className="btn-mini" onClick={()=>confirmPayment(u.uid)}>Confirm</button>}
              </div>
            </div>
          </div>
        );
